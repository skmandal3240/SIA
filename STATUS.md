...
| P6 — Harden | 74% | Privacy egress test + encrypted-at-rest audit log/memory stores (crypto-shred via device keystore) + OTA manifest + `./scripts/install.sh` / `./bin/sia` installer; `make p6-harden`; still needs OTA adapter update wired end-to-end |
